...
NexDSButton btManBlow = NexDSButton(1, 19, "btManBurn");
...
#define STEP_PIN_DOOR 22
#define DIR_PIN_DOOR 23
...
const int STEP_PIN_BURN = 26;
const int DIR_PIN_BURN = 27;
const int STEP_PIN_BDOOR = 28;
const int DIR_PIN_BDOOR = 29;
...
// AC Motor / Relay Pins
const int RELAY_BURNER = 34;
const int RELAY_IGNITION = 32;
...
void hSpeedPopCallback(void *ptr) {
  ...
  motorConfigs[selectedMotorIdx].speed = speed;  // Update variabel konfigurasi
  ...
  else if (selectedMotorIdx == 2) stepperBurner.setMaxSpeed(val*5);
...
void hStepPopCallback(void *ptr) {
  ...
  long steps = map(val, 0, 100, 0, 10000);
...
void checkIgnitionPriming() {
  ...
  if (millis() - ignitionDelayStart >= IGNITION_DELAY_MS) {
    ...
    if (remaining != last_remaining) {
      ...
      char timeBuf[10];
      sprintf(timeBuf, "%02ld:%02ld", remaining / 60, remaining % 60);
...
void loadSettings() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (motorConfigs[i].speed < 0 || motorConfigs[i].speed > 50000 || motorConfigs[i].steps < 0 || motorConfigs[i].steps > 200000) {
...
void bBlow3PopCallback(void *ptr) {
  updateBlowerState(2);
}
...
void btManBlowPopCallback(void *ptr) {
  uint32_t val;
  btManBlow.getValue(&val);
  digitalWrite(RELAY_BURNER, (val == 1) ? LOW : HIGH);  // Cek active low/high relaymu
  Serial.println(val);
  Serial.println(RELAY_BURNER);
...
void hManStepPopCallback(void *ptr) {
  ...
  manualSteps = map(val, 0, 20, 0, 2000);
}
...
void runManualStepper(AccelStepper &motor) {
  ...
  digitalWrite(ENABLE_PIN, LOW);
  motor.setMaxSpeed(manualSpeed*5);
...
void bManDoorPopCallback(void *ptr) {
  runManualStepper(stepperDoor);
  Serial.println("Mandoor");
}
void bManBDoorPopCallback(void *ptr) {
  runManualStepper(stepperBurnDoor);
}
void bManBurnPopCallback(void *ptr) {
  runManualStepper(stepperBurner);
}
void bManPushPopCallback(void *ptr) {
  runManualStepper(stepperPush);
  Serial.println("Pusher");
}
void bManAshPopCallback(void *ptr) {
  runManualStepper(stepperAsh);
}
void bManMainCPopCallback(void *ptr) {
  runManualStepper2(stepperMainConveyor);
}
...
void showTimeSet() {
  ...
  uint32_t menit = burnerActiveTimeSec ;
  sprintf(buf, "%lu sec", (unsigned long)menit);  // "15 min"
...
void bTimeUpPopCallback(void *ptr) {
  burnerActiveTimeSec += 5;  // +5 menit
  showTimeSet();
}
void bTimeDownPopCallback(void *ptr) {
  if (burnerActiveTimeSec >= 5) burnerActiveTimeSec -= 5;
  showTimeSet();
}
...
void nTimeSetPopCallback(void *ptr) {
  ...
  burnerActiveTimeSec = menit;           // simpan dalam detik TODO
  EEPROM.put(EEPROM_ADDR_TIME_SET, burnerActiveTimeSec);
  showTimeSet();
}
...
void handleIdle() {
  ...
  if (stepperDoor.currentPosition() != POS_DOOR_CLOSE) {
    digitalWrite(ENABLE_PIN, LOW);
    stepperDoor.moveTo(POS_DOOR_CLOSE);  //TODO
  } else {
...
#if USE_STEPPER
  digitalWrite(ENABLE_PIN, LOW);
  stepperDoor.moveTo(POS_DOOR_OPEN);  //TODO
...
#if USE_STEPPER
  stepperPush.moveTo(POS_PUSH_IN);  //TODO
...
#if USE_STEPPER
  stepperPush.moveTo(POS_PUSH_OUT);  //TODO
  if (stepperPush.distanceToGo() == 0) {
...
#else
  stepperDoor.setCurrentPosition(POS_DOOR_CLOSE);  //TODO
  tareScale();
  currentState = OPENING_BURN_DOOR;
#endif
}
...
void handleBurnerIn() {
#if USE_STEPPER
  long inPos = getOpenPos(2);
  stepperBurner.setAcceleration(200);
  stepperBurner.moveTo(inPos);
  if (stepperBurner.distanceToGo() == 0 && stepperBurner.currentPosition() == inPos) {
...
void handleIgniting() {
  ...
  if (!isPriming) {
    digitalWrite(RELAY_BURNER, LOW);
    digitalWrite(RELAY_IGNITION, HIGH);  // Trigger Spark
...
#if USE_STEPPER
  long closePos = getClosePos(2);
  stepperBurner.moveTo(closePos);
  if (stepperBurner.distanceToGo() == 0 && stepperBurner.currentPosition() == closePos) {
...
#if USE_STEPPER
  long closePos = getClosePos(3);
  // stepperBurnDoor.setPinsInverted(true,false,false);
  stepperBurnDoor.moveTo(closePos);
  if (stepperBurnDoor.distanceToGo() == 0 && stepperBurnDoor.currentPosition() == closePos) {
    if (oneShotAutoMode) {
      currentState = IDLE;
